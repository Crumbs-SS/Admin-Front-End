...
let DropDownList = class DropDownList extends DropDownBase {
    ...
    removeHover() {
        if (this.list) {
            const hoveredItem = (this.isServerBlazor && this.popupObj && this.popupObj.element) ?
    ...
    incrementalSearch(e) {
        if (this.liCollections.length > 0) {
            const li = incrementalSearch(e.charCode, this.liCollections, this.activeIndex, true, this.element.id, this.isServerBlazor);
    ...
    dropDownClick(e) {
        ...
        if (!this.readonly) {
            ...
            const duration = (isBlazor()) ? 1000 : (this.element.tagName === this.getNgDirective() && this.itemTemplate) ? 500 : 100;
    ...
    setValueTemplate() {
        ...
        const templateData = (isBlazor()) ? JSON.parse(JSON.stringify(this.itemData)) : this.itemData;
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const valueCompTemp = compiledString(templateData, this, 'valueTemplate', this.valueTemplateId, this.isStringTemplate, null, this.valueTempElement);
    ...
    renderPopup() {
        ...
        this.trigger('beforeOpen', args, (args) => {
            if (!args.cancel) {
                const popupEle = (this.serverPopupEle) ? this.serverPopupEle : this.createElement('div', {
                ...
                const popupInstance = (isBlazor() && this.isServerRendered) ? null : this.popupObj;
    ...
    destroyPopup() {
        const popupHolderEle = select('#' + this.element.id + '_popup_holder', document);
    ...
    showPopup() {
        ...
        const popupHolderEle = !this.isFiltering() || select('#' + this.element.id + '_popup_holder', document);
        const isDropdownComp = this.getModuleName() === 'dropdownlist' || !this.isFiltering();
    ...
    hidePopup(e) {
        ...
        const isHeader = (this.headerTemplate) ? true : false;
        const isFooter = (this.headerTemplate) ? true : false;
        ...
        this.closePopup();
        const dataItem = this.getItemData();
        const isSelectVal = this.isServerBlazor ? !isNullOrUndefined(this.value) : !isNullOrUndefined(this.selectedLI);
                ...
                case 'showPopupButton':
                    if (this.showPopupButton) {
                        if (!this.isServerBlazor) {
                            ...
                            const button = this.inputWrapper.container.querySelector('.e-input-group-icon.e-ddl-icon');
                            this.inputWrapper.buttons[0] = button;
                        }
...
let MultiSelect = class MultiSelect extends DropDownBase {
    ...
    onActionComplete(ulElement, list, e, isUpdated) {
        ...
        if (isBlazor() && this.isServerRendered && this.isDynamicDataChange && this.value !== null && this.value.length > 0) {
            ...
            for (let k = 0; k < this.value.length; k++) {
                const itemsData = this.getDataByValue(this.value[k]);
                if (itemsData) {
                    // eslint-disable-next-line
            ...
            this.trigger('removing', eventArgs, (eventArgs) => {
                if (eventArgs.cancel) {
                    ...
                    if (isBlazor() && this.isServerRendered) {
                        // eslint-disable-next-line max-len
                        const removedValues = [].concat([], removeVal);
    ...
    updateMainList(state, value) {
        if (this.allowFiltering || this.mode === 'CheckBox') {
            const element2 = this.findListElement(this.mainList, 'li', 'data-value', value);
            if (element2) {
                if (state) {
                ...
                for (let index = 0; !isNullOrUndefined(this.value[index]); index++) {
                    ...
                    if ((wrapperleng + downIconWidth + this.clearIconWidth) > overAllContainer) {
                        ...
                        while (((wrapperleng + remainSize + downIconWidth + this.clearIconWidth) > overAllContainer) && wrapperleng !== 0
                            ...
                            const textArr = this.viewWrapper.innerHTML.split(this.delimiterChar);
                            textArr.pop();
...
class CheckBoxSelection {
    ...
    listSelection(args) {
        let target;
        const isBlazorListbox = isBlazor() && (args.module && args.module === 'listbox');
...
let ListBox = ListBox_1 = class ListBox extends DropDownBase {
    ...
    onActionComplete(ulElement, list, e) {
        ...
        if (list.length === 0) {
            const noRecElem = ulElement.getElementsByClassName('e-list-nrt')[0];
            if (noRecElem) {
    ...
    triggerDrag(args) {
        ...
        if (listObj && listObj.listData.length === 0) {
            const noRecElem = listObj.ulElement.getElementsByClassName('e-list-nrt')[0];
            if (noRecElem) {
            ...
            if (this.allowDragAll) {
                ...
                selectedOptions.forEach((value) => {
                    if (value !== dropValue) {
                        ...
                        const lists = [].slice.call(this.ulElement.getElementsByClassName(cssClass.li));
                        const refChild = this.ulElement.removeChild(lists[args.currentIndex]);
        ...
        if (elems.length) {
            ...
            if (!isBlazor()) {
                ...
                if (tListBox.listData.length === 0) {
                    const noRecElem = tListBox.ulElement.getElementsByClassName('e-list-nrt')[0];
                    if (noRecElem) {
    ...
    moveAllData(fListBox, tListBox, isKey, index) {
        ...
        if (!isBlazor()) {
            ...
            if (isRefresh) {
                const noRecElem = fListBox.ulElement.getElementsByClassName('e-list-nrt')[0];
                if (noRecElem) {
